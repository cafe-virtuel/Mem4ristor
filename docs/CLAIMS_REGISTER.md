...
| C07b | pairwise synchrony lattice 10×10 FULL | 0.0197 ± 0.0142 | `experiments/verify_table1_preprint.py` (lattice_10x10) | 10 | 2026-05-06 | ✅ vérifié — **valeur réelle lattice** (différente de C07 = BA m=3). **31/07** : c'est ce claim que le Guardian appelait `C07` — collision d'étiquette corrigée dans `claims_mapping.json` (clé `C07` → `C07b`), aucun chiffre vérifié modifié. `AUDIT_LOG.md:1284` nommait déjà cette quantité C07b le 12/06 : le registre avait raison, le mapping avait la mauvaise étiquette. ⚠️ **Dette ouverte** : `figures/p2_table1_sync.csv` est versionné mais **aucun script du dépôt ne le régénère** — son producteur est `.brain/gen_c07_csv.py` (dépôt pilote), dont le chemin source pointe vers `mem4ristor-v2-main`, répertoire **mort** |
