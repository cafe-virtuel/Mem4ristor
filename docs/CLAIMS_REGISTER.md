...
|---|-------|--------|--------|-------|-------------------|--------|
| C01 | H_stable lattice 10×10 | **4.09 ± 0.19 bits** (valeur publiée : `tab:scaling` L206 et `tab:benchmarks` L521) | `experiments/verify_table1_preprint.py` → `figures/p2_table1_lattice.csv` | 10 | 2026-07-08 | ✅ vérifié — **CORRIGÉ le 31/07** : le registre portait encore `4.06 ± 0.08`, valeur d'avant la régénération du 08/07 (`cold_start=True`), **que le preprint ne cite plus nulle part**. C'est exactement le défaut de C02 et C03, corrigés le 30/07 par la passe soustractive — **C01 avait été sauté** : la correction a été appliquée aux lignes trouvées, pas à l'affirmation. Le claim Guardian visait `p2_delta_sweep.csv` (δ=0 → 4.157, **valeur jamais publiée** — ce que le `tex_guardian` signalait en N2 depuis sa création) ; il est repointé sur `p2_table1_lattice.csv` (lattice_10x10 = 4.086280 ± 0.186014) et **ancré aux deux tableaux qui la publient** (`scaling-h-10x10`, `bench-h-this-work`). `p2_delta_sweep.csv` reste valide chez lui : il n'est simplement pas la source du chiffre publié |
| C02 | H_stable lattice 4×4 | 3.21 ± 0.12 bits | `experiments/verify_table1_preprint.py` | 10 | 2026-07-08 | ✅ vérifié — **régénéré le 08/07 (backlog A4, `cold_start=True`)** : 3.22 → 3.205 (CSV). Registre mis à jour le 30/07 par la passe soustractive : il portait encore la valeur du 06/05 alors que le preprint avait suivi dès le 08/07. |
| C03 | H_stable lattice 25×25 | 4.40 ± 0.09 bits | `experiments/verify_table1_preprint.py` | 10 | 2026-07-08 | ✅ vérifié — **régénéré le 08/07 (backlog A4, `cold_start=True`)** : 4.28 → 4.404 (CSV), l'écart le plus fort des trois tailles (+0.12), std 0.06 → 0.085. Registre mis à jour le 30/07 par la passe soustractive : **il portait encore 4.28 depuis le 06/05**, soit trois semaines de décalage avec le preprint et le CSV. |
| C04 | sync FULL vs FROZEN (séparation complète) | FULL=0.0072, FROZEN=0.6582 | `experiments/p2_sigma_social_ablation.py` | **10** | 2026-06-12 | ✅ **RÉGÉNÉRÉ 12/06 (Option A), puis monté à 10 seeds** : FULL strictement stable (0.0072 aux deux tailles), FROZEN 0.6513→0.6582. Preprint corrigé (« +985% » → « ~90-fold, 0.007 → 0.658 »). Anciennes valeurs 0.067/0.730 = ancien bruit (pré-818cf67). Voir AUDIT-024. **30/07 : le libellé disait « (~×90) »** — le ratio a été retiré du preprint (dénominateur ≈ 0, statistique fragile ; décision du 08/07 appliquée partout le 30/07). Le résultat s'énonce désormais comme une **séparation complète** (Cohen d ≈ 9, 30 graines). Les deux valeurs, elles, sont inchangées. |
...
| ~~C07~~ | ~~pairwise synchrony BA m=3 FORCED FULL~~ | ~~0.031 ± 0.034~~ `[valeur morte]` | ~~`experiments/ablation_coordination_topology_sweep.py`~~ | ~~10~~ | ~~2026-05-06~~ | **🔁 REMPLACÉ par C14 le 2026-07-31** (décision de Julien) — la valeur vient du CSV du **26/04**, antérieur au fix de bruit Euler-Maruyama du 01/05 (AUDIT-024). La **même quantité** (BA m=3, FORCED, FULL) a été régénérée le 29/07 → **0.002312**, dans un CSV versionné et reproductible bit à bit, et elle est protégée par **C14**. Son script source vit dans `experiments/scratch/` (gitignoré) : le claim n'était plus régénérable par qui clone le dépôt. Chiffre conservé exprès comme trace historique, acquitté par le `tex_guardian` via le marqueur `[valeur morte]` |
| C07b | pairwise synchrony lattice 10×10 FULL | **0.036 ± 0.023** (Cold Start) | `experiments/verify_table1_preprint.py` (lattice_10x10) | 10 | 2026-07-31 | ✅ vérifié — **valeur réelle lattice** (différente de C07 = BA m=3). **Deux corrections le 31/07** (décision de Julien). **(1) Étiquette** : c'est ce claim que le Guardian appelait `C07` — corrigé dans `claims_mapping.json` (`C07` → `C07b`) ; `AUDIT_LOG.md:1284` le nommait déjà C07b le 12/06, le registre avait raison. **(2) Protocole — le vrai défaut** : la valeur publiée était `0.0197 ± 0.0142` `[valeur morte]`, produite par `.brain/gen_c07_csv.py` **sans `cold_start`**, alors que le papier revendique le Cold Start Protocol et affirmait en trois endroits (L182, L195, L515) que les deux colonnes de `tab:benchmarks` viennent d'un seul protocole. Le CSV n'était pas faux — il se rejouait **au bit près** — mais il ne venait pas du protocole annoncé. Mesuré : **+0.0162, au-delà de l'écart-type publié**. `p2_table1_sync.csv` est désormais écrit par `verify_table1_preprint.py` dans le **même run** que l'entropie ; la phrase publiée est devenue vraie au lieu d'être corrigée. Gate de fidélité : `p2_table1_lattice.csv` **identique au bit près** (SHA256) après l'ajout. ✅ **Dette de régénération FERMÉE** — le CSV était versionné sans qu'aucun script du dépôt ne le produise |
...
| S05 | **Transduction photonique — tolérance au bruit de photons (Vague 2)** | Signatures des 2 régimes intactes jusqu'à Λ=10 photons/nœud/pas (bruit 32 %) ; fonctionnel tient même à Λ=3 (58 %) ; dead zone immune (H_cog=0.0000 partout). **Λ_min ≈ 10 = chiffre de dimensionnement** | `experiments/photonic_transduction_poc.py` | 10 | 🔍 exploratoire (dossier hardware, 12/06/2026) — prolonge le claim [12] : le bruit de grenaille poissonnien ne discrimine pas plus que le gaussien. Voir `docs/hardware/PHOTONIC_PATHWAY.md` |
| S06 | **Couplage photonique + TOUT-OPTIQUE (Vague 2, étape 1 pathway)** | Couplage optique seul : intact jusqu'à Λ=3 (58 % de bruit sur le canal du MÉCANISME). Tout-optique (stimulus+couplage) : intact dès Λ=10. Dead zone : H_cog=0.0000 sur les 12 conditions | `experiments/photonic_coupling_poc.py` (sous-classe vérifiée identique bit à bit au parent quand canal parfait) | 10 | 🔍 exploratoire (12/06/2026) — **un Mem4ristor entièrement photonique est viable à ~10 photons/nœud/pas par canal**. Interprétation : u (τ_u=10) agit en filtre passe-bas naturel sur le grain photonique. Voir `docs/hardware/PHOTONIC_PATHWAY.md` §4bis |
| S07 | **Chaîne GST réaliste : saturation + inertie (Vague 2, étape 2 pathway)** | **36/36 conditions OK** : saturation jusqu'à s=3 × inertie jusqu'à τ_mat=100 pas (à Λ=10). Fonctionnel intact, dead zone H_cog=0.0000 partout | `experiments/photonic_gst_transduction_poc.py` | 10 | 🔍 exploratoire (12/06/2026) — ⚠️ stimulus STATIONNAIRE : l'inertie filtre le bruit sans rien retarder ; aucune contrainte de bande passante matériau en stationnaire (WO₃ « lent » redevient candidat). Stimuli dynamiques = étape 2bis (S08). T(P) générique, courbe GST réelle branchable. Voir `PHOTONIC_PATHWAY.md` §4ter |
| S09 | **Variabilité de fabrication optique (étape 4 pathway)** | Aucun mode de défaillance : sync ≤ 0.012 partout, l'effet dominant est une HAUSSE de H_cont (+0.05 à +0.75 — quasi-hérétiques de gain). Spec 3 zones : σ_fab ≤ 5 % nominal ; ≤ 20-30 % qualitatif ; ≥ 30 % la dead zone s'érode (H_cog 0→0.097 à σ=0.5) | `experiments/photonic_fabrication_poc.py` | 10 | 🔍 exploratoire (12/06/2026) — confirme physiquement « l'imperfection est une ressource » ; cohérent avec la tolérance SPICE au mismatch. Photonique intégrée (quelques %) = zone verte. Voir `PHOTONIC_PATHWAY.md` §4quater + bilan du quatuor |
| S08 | **Événement dynamique à travers la chaîne GST (étape 2bis)** | Effet réseau transmis à ~100 % pour τ_mat ∈ [0, 300] (même à amplitude effective 0.47) ; i_eff_max suit la prédiction 1−exp(−T/τ). Spec : I_event·(1−exp(−T/τ)) ≥ seuil (~0.5) | `experiments/photonic_event_poc.py` | 10 | 🔍 exploratoire (12/06/2026). 🚨 **Découverte collatérale : la référence ELEC (code actuel) donne dH=−0.764 vs +1.20 du claim [13] d'avril — SIGNE INVERSÉ par le changement de bruit (AUDIT-024).** ✅ **RÉSOLU 11/07/2026 : grille COMPLÈTE rejouée** (`experiments/event_phase_transition_rerun_20260711.py`, protocole d'avril inchangé, sorties datées séparées) : **[13] ne se reproduit pas — 0/9 configs du claim positives, 9/9 négatives (dH≈−1.0)** ; sur la dead zone m=5 l'effet devient ~nul. [13] RÉVISÉ dans PROJECT_STATUS. Le preprint ne cite pas [13] — soumission non affectée |
...
**C07 / C07b** *(mis à jour le 2026-07-31)* : la distinction reste vraie, et l'action qu'elle demandait est **faite**. Le claim historique C07 — « pairwise synchrony = 0.031 ± 0.034 » `[valeur morte]` — référait à **BA m=3 en régime FORCED** (I_stim=0.5), **PAS au lattice** ; la valeur lattice 10×10 était alors donnée à 0.0197 ± 0.0142 `[valeur morte]`, 36 % plus faible (elle vaut **0.036 ± 0.023** depuis le passage au Cold Start le 31/07 — voir C07b ci-dessus). Le preprint a été clarifié le **30/07** : `tab:benchmarks` se contredisait lui-même (corps « lattice 10×10 », légende « BA m=3 ») et publiait le chiffre BA ; il porte désormais les valeurs lattice, les deux colonnes venant enfin des mêmes runs (décision de Julien). Depuis le **31/07**, C07 est remplacé par **C14** (voir le tableau ci-dessus) et le Guardian n'appelle plus « C07 » ce qui est C07b.
