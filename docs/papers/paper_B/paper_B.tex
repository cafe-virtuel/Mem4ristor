...
\subsection{Physical Scaling and Hardware Budgets}
To evaluate the practical scalability of our neuromorphic oscillator mapping, we analyze the physical execution times and energy budgets across three candidate hardware substrates, anchoring the model's unit time step ($dt = 0.05$) to their intrinsic physical periods ($T_{\mathrm{node}}$):
\begin{enumerate}
    \item \textbf{Spintronics (STNO vortex)}: Mapping the membrane potential $v$ to a Spin-Torque Nano-Oscillator (STNO) with an intrinsic period $T_{\mathrm{node}} = 1$ ns yields a physical integration step $dt_{\mathrm{phys}} = 2.25$ ps. Running a 3000-step network trajectory of $N=100$ nodes requires only $6.75$ ns of physical emulation time and consumes $\sim 2.02$ nJ (based on a nominal power of 3 mW per vortex).
    \item \textbf{Photonics (GST phase-change)}: Mapping the doubt state $u$ to a multi-level Ge$_2$Sb$_2$Te$_5$ (GST) optical cell with $T_{\mathrm{node}} = 100$ ns yields $dt_{\mathrm{phys}} = 225$ ps. The 3000-step trajectory requires $675$ ns and consumes a signal-only energy of $\sim 384$ fJ (at a budget of 10 photons per step at 1550 nm).
    \item \textbf{Electrical (NbO$_2$ Mott Neuristor)}: An alternative electrical oscillator with $T_{\mathrm{node}} = 1$ $\mu$s yields $dt_{\mathrm{phys}} = 2.25$ ns, completing the trajectory in $6.75$ $\mu$s while consuming $\sim 67.5$ nJ (assuming a nominal 225 fJ per node-step).
\end{enumerate}
Compared to a digital CPU simulation (which requires $\sim 1.0$ s and consumes $\sim 60$ J on a standard 65W processor), these hardware pathways offer a $10^5\times$ to $10^8\times$ speedup and a $10^9\times$ to $10^{14}\times$ reduction in energy consumption. This illustrates that physical analog emulation is not merely an implementation detail, but a major computational paradigm shift.
