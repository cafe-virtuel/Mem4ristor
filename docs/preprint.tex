...
\textbf{Scope note (audit 2026-04-22).} The argument above is \emph{vacuous} when $I_{\text{stimulus}} = 0$, which is the default for all lattice experiments (Section~\ref{sec:results_lattice}). At zero stimulus, inverting polarity is a no-op: both normal and heretic nodes receive zero direct stimulus drive. Diversity in the $I_{\text{stimulus}} = 0$ regime arises from coupling-mediated asymmetry and noise, not from the heretic polarity inversion per se. The lower bound of Eq.~\ref{eq:entropy_bound} and the claim $H_{\text{stable}} \approx 0.92$ should be read as applying to the $I_{\text{stimulus}} \neq 0$ operating regime only.
...
\begin{table}[ht]
\centering
\caption{Scaling performance on 2D periodic lattices (Cold Start, $\eta = 0.15$, 7 seeds per configuration). \textbf{\textcolor{red}{⚠ Pre-KIMI bins}} — these $H_{\text{stable}}$ values were computed with the pre-KIMI thresholds ($\pm 0.8$, $\pm 1.5$), not the current thresholds ($\pm 0.4$, $\pm 1.2$, Table~\ref{tab:states}). With current thresholds and $I_{\text{stimulus}} = 0$, the system converges to a single bin and $H_{\text{stable}} \approx 0$ (audit 2026-04-22).}
\label{tab:scaling}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[ht]
\centering
\caption{Ablation study ($10 \times 10$ lattice, Cold Start, 3000 steps, 5 seeds). $H_{\text{stable}}$ is the mean over the last 25\% of each run. \textbf{\textcolor{red}{⚠ Pre-KIMI bins}} — values computed with obsolete thresholds ($\pm 0.8$, $\pm 1.5$); see Table~\ref{tab:scaling} caption for details (audit 2026-04-22).}
\label{tab:ablations}
\begin{tabular}{lcc}
\toprule
...
\begin{table}[ht]
\centering
\caption{Sustained entropy $H_{\text{stable}}$ by BA attachment parameter $m$ and coupling normalization. ``$\star$'' denotes $H > 0.5$. \textbf{\textcolor{red}{⚠ Pre-KIMI bins}} — see Table~\ref{tab:scaling} caption (audit 2026-04-22).}
