...
\textbf{Doubt and Modulatory Variables.} The concept of a slow modulatory variable controlling the fast dynamics of a network has roots in computational neuroscience, particularly in models of neuromodulation and homeostatic plasticity. In our architecture, the doubt variable ($u$) acts as a localized prediction error, a concept that mathematically mirrors the minimization of surprise in Friston's Free Energy Principle~\cite{friston2010}.

\textbf{Topological Phase Transitions.} The dependence of synchronization on network topology, particularly the algebraic connectivity (Fiedler value, $\lambda_2$), is well established~\cite{pikovsky2001}. However, our observation of a continuous phase transition \emph{out} of functional diversity driven purely by topological redundancy (irrespective of local degree normalization) extends the understanding of how dense scale-free networks naturally suppress state heterogeneity.
% ============================================================================
% 2. MODEL
% ============================================================================
\section{Model}
...
\begin{enumerate}
    ...
    \item \textbf{Binder Cumulant ($U_4$)}: A fourth-order statistical measure $U_4 = 1 - \frac{\langle H_{\text{stable}}^4 \rangle}{3\langle H_{\text{stable}}^2 \rangle^2}$ used in finite-size scaling to pinpoint thermodynamic phase transitions.
\end{enumerate}
...
For each network realization, we define the effective order parameter $m$ from the stable entropy measure $H_{\text{stable}}$, calculated as the time-average of continuous spatial snapshot entropies (defined in Section~\ref{sec:model}) over the tail steps (the last 25\% of the simulation). We compute the fourth-order cumulant

\begin{equation}
U_4(\lambda_2) = 1 - \frac{\langle m^4 \rangle}{3\langle m^2 \rangle^2},
\end{equation}

where $\langle \cdot \rangle$ denotes the ensemble average over $N_{\text{real}} = 40$ independent initial conditions and topological realizations at fixed algebraic connectivity $\lambda_2$. To ensure size-independent connectivity sweeps, random edge additions are scaled according to the scaling law $p_{\text{add}} = c / N$ with $c \in [0.0, 15.0]$. For each configuration, the network is evolved under an active heretics regime ($I_{\text{stimulus}} = 0.5$) for $T_{\text{eq}} = 1875$ time units to discard transients, and statistics are collected over $T_{\text{stat}} = 625$ time units (totaling $2500$ steps with $\Delta t = 0.05$).

The Binder cumulant exhibits a distinct minimum that deepens and converges with increasing system size, consistent with finite-size scaling theory for a continuous phase transition. Across network sizes $N \in \{100, 200, 400\}$, the cumulant curves converge toward a transition point centered near $\lambda_{2,\text{crit}} \approx 2.31$ (Fig.~\ref{fig:binder_cumulant}). The corresponding order parameter $\langle H_{\text{stable}} \rangle$ exhibits a smooth, size-independent drop, confirming the physical nature of the transition. Interestingly, the active heretics regime prevents an absolute consensus collapse, sustaining a non-zero "remnant" diversity ($H \approx 2.4$ bits) even at high connectivities, which is a classic signature of a glass-like arrested state. While residual finite-size effects remain visible at lower $N$, the convergence trend supports the physical interpretation of the Dead Zone as a genuine thermodynamic phase. Error bars represent the standard error of the mean across the 40 realizations.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../figures/v6_binder_cumulant.png}
\caption{
\textbf{Finite-size scaling and data collapse of the thermodynamic transition.} 
(\textit{Left}) Binder cumulant $U_4 = 1 - \langle m^4 \rangle / (3\langle m^2 \rangle^2)$ as a function of the algebraic connectivity $\lambda_2$ for network sizes $N \in \{100, 200, 400\}$. 
Each data point averages independent realizations after transient discard. 
With increasing $N$, the $U_4$ curves collapse, converging toward the thermodynamic critical point $\lambda_{2,\text{crit}} \approx 2.31$ (dashed red line). 
(\textit{Right}) Corresponding order parameter $\langle H_{\text{stable}} \rangle$ showing a size-independent universal entropy drop across the critical region, signaling the loss of functional degrees of freedom and the onset of glass-like arrest, while sustaining a remnant diversity due to the active heretics. 
The inset displays the numerical derivative $|d\langle H \rangle / d\lambda_2|$ for $N=400$, confirming the critical divergence centered on $\lambda_{2,\text{crit}} \approx 2.31$.
}
...
\subsection{Thermodynamic Viability and Active Inference}
...
Yet, the FEP also implies that this homeostasis has physical limits: active inference is only viable within a bounded region of the state space. When the density of topological constraints overwhelms the dissipative capacity of the local $u_i$-mediated decoupling (i.e., when $\lambda_2$ exceeds the numerically observed critical threshold $\lambda_{2,\text{crit}} \approx 2.31$), the pervasive, non-local influences override the local capacity for active inference. The prediction error $|\mathcal{L}_v|$ becomes uniformly high across the network, forcing $u_i \to 1$ globally.

We term this global saturation a \textbf{thermodynamic collapse}: the network undergoes a transition into a state of maximal rigidity where the continuous entropy production drops to a residual baseline ($H \to H_{\text{res}} \approx 2.4$), analogous to a glass-like arrest of its functional degrees of freedom. This is rigorously supported by the Binder cumulant analysis in Section~\ref{sec:binder_cumulant}, which confirms the emergence of a sharp finite-size transition, with the $U_4$ minimum converging toward $\lambda_{2,\text{crit}}$ as $N \to \infty$, characteristic of a continuous (second-order) phase transition into a frozen, non-dissipative state.
