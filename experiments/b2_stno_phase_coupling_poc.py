...
def bootstrap_ci(a, b, n_boot=50000, seed=0):
    ...
    n_boot 5000 -> 50000 le 31/07/2026 : a 5000 tirages, l'erreur de Monte-Carlo sur
    chaque borne valait ~0.0013 (mesure : 60 tirages sur les memes donnees donnaient
    SIX valeurs distinctes a la 3e decimale). A 50000 elle tombe a ~0.0004, donc les
    bornes publiees a 3 decimales ne dependent plus du tirage. Voir aussi le seed."""
    rng = np.random.RandomState(seed)
...
def main():
    ...
    for topo_name in topologies:
        ...
        for gain in GAINS_U:
            ...
            # l'interpreteur (PYTHONHASHSEED, protection anti-collision depuis Py3.3).
            # Ce `seed=` avait donc toutes les apparences d'une graine fixee sans en
            # etre une : les IC changeaient a chaque execution. crc32 est deterministe
            # et conserve l'intention d'origine (un tirage propre a chaque cellule).
