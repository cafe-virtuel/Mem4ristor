...
if __name__ == '__main__':
    ...
    for m in M_TARGETS:
        ...
        for alpha in ALPHAS:
            ...
            for seed in SEEDS:
                ...
                h = run_experiment(adj, alpha, seed)
                h_list.append(h)
            h_mean = np.mean(h_list)
            h_std = np.std(h_list)
            star = " ***" if h_mean > 0.5 else ""
            print(f"  alpha={alpha:.1f}  H={h_mean:.4f} +/- {h_std:.4f}{star}")
            results[(m, alpha)] = (h_mean, h_std, h_list)
    ...
    # Best alpha per topology
    print(f"\nOptimal alpha per topology:")
    for m in M_TARGETS:
        best_alpha = max(ALPHAS, key=lambda a: results[(m, a)][0])
        best_h = results[(m, best_alpha)][0]
        print(f"  BA m={m}: alpha*={best_alpha:.1f} -> H={best_h:.4f}")
